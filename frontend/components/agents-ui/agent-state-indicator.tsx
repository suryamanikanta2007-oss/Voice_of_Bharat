...
import { PhoneCallIcon, MicrophoneIcon, SpeakerHighIcon, SpinnerIcon, PhoneDisconnectIcon, SparkleIcon } from '@phosphor-icons/react';
import { cn } from '@/lib/shadcn/utils';

export type AgentDisplayState = 'ready' | 'connecting' | 'listening' | 'speaking' | 'thinking' | 'disconnected';
    ...
    switch (state) {
      case 'ready':
        return {
          ...
          subtitle: isHi ? 'सहायक अनीशा से बात करने के लिए बटन दबाएं' : 'Click start to talk with Anisha',
      ...
      case 'connecting':
        return {
          ...
          subtitle: isHi ? 'कृपया प्रतीक्षा करें, सुरक्षित कनेक्शन बनाया जा रहा है' : 'Please wait, connecting to secure helpline',
      ...
      case 'listening':
        return {
          ...
          icon: <MicrophoneIcon className="size-5 text-blue-500 animate-bounce" />,
        };
      case 'speaking':
        return {
          ...
          icon: <SpeakerHighIcon className="size-5 text-purple-500 animate-pulse" />,
        };
      case 'thinking':
        return {
          ...
          icon: <SparkleIcon className="size-5 text-orange-500 animate-spin" />,
        };
      case 'disconnected':
      default:
        return {
          title: isHi ? 'कॉल समाप्त हो गई' : 'Call Ended',
          subtitle: isHi ? 'नया सत्र शुरू करने के लिए स्टार्ट पर क्लिक करें' : 'Click below to start a new session',
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          'flex items-center justify-between gap-3 px-4 py-2.5 rounded-2xl border backdrop-blur-md shadow-sm transition-all',
          details.color,
          className
        )}
      >
        <div className="flex items-center gap-3">
          <div className="relative flex items-center justify-center size-9 rounded-xl bg-background/80 shadow-xs border border-border/30">
            {details.icon}
            <span className={cn('absolute -top-0.5 -right-0.5 size-2.5 rounded-full', details.dotColor)} />
          </div>
          <div className="flex flex-col text-left">
            <span className="font-bold text-sm tracking-tight text-foreground leading-tight">
              {details.title}
            </span>
            <span className="text-[11px] opacity-80 font-sans font-medium">
              {details.subtitle}
            </span>
          </div>
        </div>
          ...
          <div className="flex items-center gap-1 h-5 px-1">
            {[0.4, 0.9, 0.6, 1, 0.5].map((delay, idx) => (
              <motion.span
                key={idx}
                animate={{
                  height: state === 'listening' ? ['6px', '18px', '8px', '20px', '6px'] : ['10px', '22px', '6px', '18px', '10px'],
                }}
                transition={{
                  duration: 0.8,
                  repeat: Infinity,
